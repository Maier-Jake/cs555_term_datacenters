...
\begin{itemize}
    \item URL: \texttt{https://www.eia.gov/electricity/data/eia861/}
    \item Coverage: 2015--2024 (10 years), all US utilities
    \item Format: Annual Excel files converted to CSV
    \item Key fields: State, Year, Revenues (\$), Sales (MWh), Customers
    \item Derived metric: Avg\_Price\_per\_kWh = Revenues / Sales
\end{itemize}

\textbf{Source 2: Data Center Locations}
\begin{itemize}
    \item Sources: Company press releases, Kaggle dataset, industry databases
...
\textbf{Challenge:} Decision Trees need cumulative totals (``How many data centers exist in state X by year Y?''), not just openings per year.
...
\subsubsection{Pipeline Execution Summary}

\textbf{Distributed Operations Count:}
\begin{itemize}
    \item \textbf{Shuffles:} 3 (groupBy aggregations)
    \item \textbf{Broadcasts:} 2 (crossJoin for grid, small table joins)
    \item \textbf{Window Functions:} 3 (cumulative sum, lag price, running totals)
    \item \textbf{Joins:} 4 (grid + byYear + prePeriod + prices)
\end{itemize}

\textbf{Computational Complexity:}
\begin{itemize}
    \item Cartesian product: $O(|states| \times |years|)$ = $O(240)$
    \item Cumulative sum: $O(n \log n)$ per partition (sorting)
    \item Joins: $O(n)$ with hash partitioning
    \item Total: $O(n \log n)$ dominated by sorting for window functions
\end{itemize}

\textbf{Why This Required Distributed Processing:}

While 240 observations is small, this pipeline demonstrates distributed systems concepts:
\begin{enumerate}
    \item \textbf{Scalability:} Identical code would handle 240,000 observations (1000x more states)
    \item \textbf{Partitioning:} Window functions leverage data locality
    \item \textbf{Fault Tolerance:} Spark lineage enables automatic retry on failure
    \item \textbf{Lazy Evaluation:} Entire pipeline optimized before execution (no intermediate materializations)
\end{enumerate}
...
\textbf{Attempt 3: Decision Tree (SUCCESS)}
\begin{itemize}
...
\bottomrule
\end{tabular}
\caption{Decision Tree model performance on held-out test set (48 observations, 20\% of data)}
\end{table}

\textbf{Interpretation of $R^2 = 0.179$:}

Our model explains 17.9\% of price variation. This may seem low, but is
consistent with economics literature for cross-sectional models with limited
features (Cohen 1988, Bellemare 2015). Electricity prices depend on many factors
(natural gas costs, weather, regulation) beyond data centers. However, we have
shown that there is at the very least a positive correlation between the number
of nearby data centers and residential electricity prices.

\subsection{Regional Case Study: Arizona}
...
\caption{Compares electricity prices in two 'high' data center
	states (California and Texas) against a 'low' data center state (Wyoming)
...
\textbf{Predictive Relationship:}
Our model demonstrates that data center metrics explain $\sim$18\% of
electricity price variation across states. This is a \textit{correlation}, not
proven causation, but provides a quantitative starting point for policy
analysis.

\textbf{Limitations:}
\begin{itemize}
    \item Decision Tree provides predictions but not statistical significance (p-values)
    \item Small sample size (240 observations) limits generalization
    \item Missing control variables (natural gas prices, weather) could confound results
...
\textbf{Performance:} RMSE = \$0.0153/kWh, $R^2 = 0.179$
...
\textbf{Trade-offs:} We sacrificed coefficient interpretation (Linear Regression) and accuracy (GBT) to achieve successful distributed execution (Decision Tree). This reflects real-world engineering constraints when deploying ML at scale.
